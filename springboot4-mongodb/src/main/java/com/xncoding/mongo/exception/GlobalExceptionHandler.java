 ...
 * <p>
 * 相比上一篇只换了两个异常类型，但换的理由很值得看清楚：
 * <ul>
 *   <li>乐观锁：JPA 抛 {@code ObjectOptimisticLockingFailureException}（Spring ORM 的类），
 *       MongoDB 抛 {@link OptimisticLockingFailureException}（Spring DAO 的类，
 *       是前者的父类）。都是"你手上的数据过期了"，接哪个取决于底层谁在管版本。</li>
 *   <li>唯一键冲突：JPA 抛 {@code DataIntegrityViolationException}，
 *       MongoDB 抛 {@link DuplicateKeyException}（它是前者的子类）。
 *       信息量更大——MongoDB 的错误码 {@code 11000} 就是"唯一索引冲突"，
 *       驱动把它翻译成了这个专门的异常。</li>
 * </ul>
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     * 乐观锁冲突：409。
     * <p>
     * 注意这里接的是 Spring DAO 的 {@link OptimisticLockingFailureException}，
     * 不是 Spring ORM 的 {@code ObjectOptimisticLockingFailureException}。
     * 后者是前者的子类，本篇的 classpath 上根本没有 spring-orm，
     * 照着上一篇抄 import 会编译不过。
     */
    ...
    public ResponseEntity<ProblemDetail> handleOptimisticLock(OptimisticLockingFailureException ex) {
        ...
        // 这里<b>没有</b> entity 属性。上一篇用的是 getPersistentClassName()，
        // 那是子类 ObjectOptimisticLockingFailureException 才有的方法，
        // 父类只有类型名。少一个字段不算损失，但"照抄上一篇编译不过"这件事
        // 本身就是两个模块异同的一个缩影——同一个语义概念，落在不同的类上，
        // 能拿到的东西就不一样。
     ...
     * 唯一索引冲突：409。
     * <p>
     * MongoDB 的错误码 11000 就是唯一索引冲突。驱动把它翻译成
     * {@link DuplicateKeyException}，比泛化的 {@code DataIntegrityViolationException}
     * 精确，所以单独接一个。异常里带得出是哪个索引被撞了。
     */
